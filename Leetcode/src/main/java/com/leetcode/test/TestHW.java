 ...
 * 返回一个二维字符串数组 answer ，其中 answer[i] = [creator[i], id[i]] 表示 creator[i] 的流行度<最高>且其<最流行>的视频 id 是 id[i],可以按任何顺序返回该结果。
 * <p>
 * 示例 1：`
 * <p>
 ...
 * <p>
 * <p>
 * public List<List<String>> answer(String[] creators, String[] ids, int[] views) {
 * <p>
 * }
 *
 * @Author: xiehongyu
 * @Date: 2024/06/24 17:05
 */
@Slf4j
public class TestHW {
    ...
    @Test
    public void test() {
        answer(new String[]{"alice", "alice", "alice"}, new String[]{"a", "b", "c"}, new int[]{1, 2, 2});
    }
    ...
    public List<List<String>> answer(String[] creators, String[] ids, int[] views) {
        ...
        for (int i = 0; i < creators.length; i++) {
            if (creatorSet.contains(creators[i])) {
                if (resMap.containsKey(creators[i])) {
                    if (views[i] > views[resMap.get(creators[i])]) {
